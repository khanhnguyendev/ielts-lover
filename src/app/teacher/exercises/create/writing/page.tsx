...
function TeacherCreateWritingExerciseContent() {
        ...
        if (duplicateId) {
            getExerciseById(duplicateId).then(ex => {
                if (ex) {
                    ...
                    notifySuccess(NOTIFY_MSGS.SUCCESS.EXERCISE_LOADED.title, NOTIFY_MSGS.SUCCESS.EXERCISE_LOADED.description, "Continue");
                }
            });
        }
    }, [searchParams]);
    ...
    async function handleAnalyzeImage() {
        ...
        try {
            ...
            const analysis = await analyzeTeacherChartImage(base64, imageFile.type);
            if (analysis.success && analysis.data) {
                const data = analysis.data as typeof imageAnalysis;
                setImageAnalysis(data);
                window.dispatchEvent(new CustomEvent('credit-change', { detail: { amount: -analysisCost } }));
                if (data?.is_valid && data.title && !title) {
                    setTitle(data.title);
                }
    ...
    async function handleGenerate() {
        ...
        try {
            const result = await generateTeacherAIExercise(type, topic || undefined, chartType);
            if (result) {
                ...
                if ('chart_data' in result && result.chart_data) {
                    setChartData(result.chart_data as Record<string, unknown>);
                }
                notifySuccess(
    ...
    async function handleSubmit(formData: FormData) {
        ...
        try {
            ...
            if (type === "writing_task1" && imageFile) {
                ...
                imageUrl = await uploadTeacherImage(uploadFormData);
            }
            ...
            notifySuccess(
                NOTIFY_MSGS.SUCCESS.EXERCISE_PUBLISHED.title,
                NOTIFY_MSGS.SUCCESS.EXERCISE_PUBLISHED.description,
                "Back to List"
            );
